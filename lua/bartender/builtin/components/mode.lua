...
mode.mode_specs = {
  ...
  replace = {
    ...
    highlight = function()
      return {
        fg = utils.get_hl_attr("String", "fg"),
        reverse = true,
      }
    end,
  },
  ...
  terminal = {
    ...
    highlight = function()
      return {
        fg = utils.get_hl_attr("Normal", "bg"),
        bg = utils.get_hl_attr("DiffAdd", "fg"),
      }
    end,
  },
}
...
function mode.get_current_mode_highlight_reversed()
  ...
  return vim.tbl_deep_extend("force", mode_component_hl, { reverse = not mode_component_hl.reverse })
end
